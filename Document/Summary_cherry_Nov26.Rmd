...
tmp2%>%
  ggplot(aes(x=product_name,y = count))+
  geom_bar(stat="identity",fill="red")+
  labs(title="Top 10 previous reordered products", x="Products", y="the number of orders")+
  theme(axis.text.x=element_text(angle=90, hjust=1))+
  theme(plot.title = element_text(hjust = 0.5))
```
...
tmp %>%
  ggplot(aes(x=department,y = count))+
  geom_bar(stat="identity",fill="red")+
  labs(title="The number of previous reordered products in each department", x="Department", y="the number of orders")+
  theme(axis.text.x=element_text(angle=90, hjust=1))+
  theme(plot.title = element_text(hjust = 0.5))
```
...
We compared the predicted results with the results of reordered products in the original data set. First, we compared the most popular products we predicted with the the most popular products people ordered before. As we predict, the top five most popular products are banana, bag of organic bananas, organic baby spinach, organic raspberries and organic strawberries. The top five products that people used to buy are banana, bag of organic bananas and organic strawberries, organic baby spinach and organic avocado. We also found that the top three departments with most number of predicted products are produce, dairy eggs and beverages, the result is the same as we found according to the previous orders. 
